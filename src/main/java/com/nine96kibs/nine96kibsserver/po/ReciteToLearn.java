...
public class ReciteToLearn extends ReciteLearnInfo implements Comparable<ReciteToLearn> {
    ...
    /**
     * 还会生成背诵优先级，该优先级公式为(b/(c+impressionCount) + d)*log(a)(deltaSecond+1)
     * 其中b = 1  c = 1  d在latestChoice为0时为1.5 其他值时为0  a = 10
     * @param learnInfo
     */
    public ReciteToLearn(ReciteLearnInfo learnInfo){
        super(learnInfo);
        if (learnInfo == null || learnInfo.getLatestLearn() == null){
            recitePrior = 1;
        }else {
     ...
     */
    @Override
    public int compareTo(ReciteToLearn o) {
        return Double.compare(recitePrior, o.recitePrior);
    }
}
